DROP FUNCTION IF EXISTS convert_conversion(VARCHAR);
...
BEGIN
	BEGIN
	table_name = 'table_name' || region_name || selected_month;
    ...
    CREATE TABLE table_name as
        ...
        INNER JOIN organisationunit region ON(district.parentid = region.organisationunitid AND region = region_name)
        ...
        ALTER TABLE table_name ADD COLUMN curr_pe_val_planted_latest_submited varchar;
        ALTER TABLE table_name ADD COLUMN curr_pe_val_productivity_latest_submited varchar;
        ALTER TABLE table_name ADD COLUMN prev_pe_val_planted_latest_submited varchar;
        ALTER TABLE table_name ADD COLUMN prev_pe_val_productivity_latest_submited varchar;

        -- Set planted area and productivity to 0 if it is null and the form has been submitted

        UPDATE table_name SET curr_pe_val_planted_latest_submited = '0' WHERE curr_pe_submission = '1' AND curr_pe_val_planted_latest_submited is null;
        UPDATE table_name SET curr_pe_val_productivity_latest_submited = '0' WHERE curr_pe_submission = '1' AND curr_pe_val_productivity_latest_submited is null;
        UPDATE table_name SET prev_pe_val_planted_latest_submited = '0' WHERE prev_pe_submission = '1' AND prev_pe_val_planted_latest_submited is null;
        UPDATE table_name SET prev_pe_val_productivity_latest_submited = '0' WHERE prev_pe_submission = '1' AND prev_pe_val_productivity_latest_submited is null;
        ...
        curr_pe_val_planted_latest_submited = (
            ...
            INNER JOIN organisationunit ou ON(ou.organisationunitid = dv.sourceid AND table_name.ou = ou.uid)
            INNER JOIN _periodstructure _ps ON(_ps.iso = table_name.curr_pe)
        ...
        prev_pe_val_planted_latest_submited = (
            SELECT dv.value FROM datavalue dv
            INNER JOIN organisationunit ou ON(ou.organisationunitid = dv.sourceid AND table_name.ou = ou.uid)
            INNER JOIN _periodstructure _ps ON(_ps.iso = table_name.prev_pe)
            INNER JOIN _periodstructure _ps2 ON(_ps2.startdate <= _ps.enddate)
            INNER JOIN completedatasetregistration submission ON(_ps2.periodid = submission.periodid AND submission.sourceid = ou.organisationunitid AND submission.datasetid = 1512)
            WHERE dv.dataelementid = dx_planted_id AND dv.categoryoptioncomboid = co_planted_id AND dv.attributeoptioncomboid = 339884
            ORDER BY _ps2.enddate DESC LIMIT 1
        ...
        ALTER TABLE table_name ADD COLUMN used_01_factor_bool integer;
        ALTER TABLE table_name ADD COLUMN val_not_from_imm_prev_pe_bool integer;

        UPDATE table_name SET curr_pe_val_planted_latest_submited = cast(cast(prev_pe_val_planted_latest_submited as decimal) + 0.1 as varchar), used_01_factor_bool = 1
        ...
        ALTER TABLE table_name ADD COLUMN exponent_diff_curr_prev_val_planted integer;
        UPDATE table_name SET exponent_diff_curr_prev_val_planted=cast(log(abs(cast(curr_pe_val_planted_latest_submited as decimal) - cast(prev_pe_val_planted_latest_submited as decimal))) as integer) WHERE cast(curr_pe_val_planted_latest_submited as decimal) - cast(prev_pe_val_planted_latest_submited as decimal) <> 0;

        -- Update exponential difference for productivity
        ALTER TABLE table_name ADD COLUMN exponent_diff_curr_prev_val_productivity integer;
        UPDATE table_name SET exponent_diff_curr_prev_val_productivity=cast(log(abs(cast(curr_pe_val_productivity_latest_submited as decimal) - cast(prev_pe_val_productivity_latest_submited as decimal))) as integer) WHERE cast(curr_pe_val_productivity_latest_submited as decimal) - cast(prev_pe_val_productivity_latest_submited as decimal) <> 0;

        -- Add new data columns

        ALTER TABLE table_name ADD COLUMN converted_curr_pe_val_planted decimal;
        ALTER TABLE table_name ADD COLUMN converted_curr_pe_val_productivity decimal;

        -- Perform migration of planted area column
        UPDATE table_name SET converted_curr_pe_val_planted = cast(curr_pe_val_planted_latest_submited as decimal) - cast(prev_pe_val_planted_latest_submited as decimal);

        -- Perform migration of productivity column
        UPDATE table_name SET converted_curr_pe_val_productivity =
        ...
        COPY (select * from table_name) TO '/tmp/name.csv' WITH CSV HEADER;
...
*/
SELECT convert_conversion('201607','iPOwn2LlQ2e');
