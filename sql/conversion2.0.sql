...
CREATE OR REPLACE FUNCTION convert_conversion(selected_month VARCHAR) RETURNS VARCHAR AS $$
DECLARE
	_c text;
	results VARCHAR;
	isoperiod VARCHAR;
...
BEGIN
    ...
    CREATE TABLE _data as
        ...
        LEFT JOIN organisationunit ou ON(ou.hierarchylevel = 4)
        LEFT JOIN organisationunit district ON(ou.parentid = district.organisationunitid)
        LEFT JOIN organisationunit region ON(district.parentid = region.organisationunitid)
        LEFT JOIN dataelement de1 ON(de1.uid=table2.column2)
        LEFT JOIN categoryoptioncombo co1 ON(co1.uid=table2.column3)
        LEFT JOIN dataelement de2 ON(de2.uid=table2.column4)
        LEFT JOIN categoryoptioncombo co2 ON(co2.uid=table2.column5)
        INNER JOIN (
        ...
        UPDATE _data SET curr_pe_val_planted_latest_submited = '0' WHERE curr_pe_submission = '1' AND curr_pe_val_planted_latest_submited is null;
        UPDATE _data SET curr_pe_val_productivity_latest_submited = '0' WHERE curr_pe_submission = '1' AND curr_pe_val_productivity_latest_submited is null;
        UPDATE _data SET prev_pe_val_planted_latest_submited = '0' WHERE prev_pe_submission = '1' AND prev_pe_val_planted_latest_submited is null;
        UPDATE _data SET prev_pe_val_productivity_latest_submited = '0' WHERE prev_pe_submission = '1' AND prev_pe_val_productivity_latest_submited is null;
...
*/
SELECT convert_conversion(2016);
...
-- COPY (select * from _data) TO '/tmp/converted_data_arusha_maize.csv' WITH CSV HEADER;
